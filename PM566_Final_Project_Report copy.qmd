---
title: "PM566 Final Project Report"
...
knitr::kable(team_metrics_by_game, col.names = custom_colnames, formalt = 'LaTeX') %>%
  kable_styling(full_width = F, position = "center",
                latex_options = c("striped", "scale_down"))
...
# Plotting Stacked Bar Chart
ggplot(team_metrics_by_game, aes(x = Activity_Name)) +
  geom_bar(aes(y = Total_High_Intensity_Bouts, fill = "High Intensity Bouts"), stat = "identity") +
  geom_bar(aes(y = Total_Explosive_Efforts, fill = "Explosive Efforts"), stat = "identity", position = "stack") +
  geom_bar(aes(y = Total_Sprints, fill = "Sprints"), stat = "identity", position = "stack") +
  labs(title = "Distribution of Physical Metrics for Each Game",
       x = "Game",
       y = "Bouts",
       fill = "Physical Metrics") +   # Added legend title here
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("High Intensity Bouts" = "lightgreen",
                               "Explosive Efforts" = "yellow",
                               "Sprints" = "blue"))
